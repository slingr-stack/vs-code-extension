...
export class NewModelTool implements AIEnhancedTool {
  ...
  public async createNewModel(targetUri: vscode.Uri | AppTreeItem, cache?: MetadataCache): Promise<void> {
    ...
    try {
      ...
      // Step 4: Ask user to select a datasource (optional)
      let selectedDataSource: string | undefined;
      if (cache) {
        const dataSources = cache.getDataSources();
        if (dataSources.length > 0) {
          const dataSourceOptions = [
            { label: "None (Skip datasource)", description: "Create model without specifying a datasource", dataSourceName: undefined },
            ...dataSources.map(ds => ({
              label: ds.name,
          ...
          const selectedOption = await vscode.window.showQuickPick(dataSourceOptions, {
            placeHolder: "Select datasource or skip"
          });
          ...
          // Set the selected datasource (undefined if "None" was selected)
          selectedDataSource = selectedOption.dataSourceName;
        }
      }
